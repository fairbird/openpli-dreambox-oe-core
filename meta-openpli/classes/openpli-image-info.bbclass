...
OPENPLI_IMAGE_INFO_ARCHIVES ?= "${IMAGE_NAME}.zip ${IMAGE_NAME}_*.zip"
OPENPLI_IMAGE_INFO_EXCLUDE ?= ""

python openpli_image_info() {
    import glob
    import os
    import subprocess
    ...
    archives = set()
    excluded = set(d.getVar('OPENPLI_IMAGE_INFO_EXCLUDE').split())
    for directory in (d.getVar('IMGDEPLOYDIR'), d.getVar('DEPLOY_DIR_IMAGE')):
        for pattern in d.getVar('OPENPLI_IMAGE_INFO_ARCHIVES').split():
    ...
    lines = ['openPLI Image Information', '========================', '']
    ...
    lines.extend([
        ...
        '',
        'Enigma2 Bug Reports:',
        'https://github.com/fairbird/enigma2-dreambox/issues',
        '',
        'Build System Bug Reports:',
        'https://github.com/fairbird/openpli-dreambox-oe-core/issues',
        '',
    ...
    info = Path(d.getVar('WORKDIR')) / 'openpli-image-info' / 'openpli-image-info.txt'
    ...
    for archive in sorted(archives):
        ...
        with ZipFile(archive) as image_zip:
            if image_zip.namelist().count(info.name) != 1 or image_zip.read(info.name) != content:
                bb.fatal('Unable to add OpenPLI image information to %s' % archive)
        bb.note('Added %s to %s' % (info.name, archive.name))
}

IMAGE_POSTPROCESS_COMMAND:append = " openpli_image_info;"
